...
rmsData = NaN(3120, 2);  % Crear una matriz de NaN de 3120x2 para almacenar RMS y etiquetas
...
for j = 1:length(folderNames)  % Recorre todas las carpetas encontradas
    ...
    % Ignorar archivos que no correspondan a señales EMG
    currentFile = filenames(i).name;

    % Ignorar archivo combinado de características
    if contains(currentFile, 'features_dataset')
        continue;
    end

    % Ignorar archivo MATLAB_TABLA_COMBINADA
    if contains(currentFile, 'MATLAB_TABLA_COMBINADA')
        continue;
    end

    % Leer el archivo Excel utilizando readtable
    dataTable = readtable(fullfile(path1, currentFile));
        ...
        filename = erase(currentFile, ".xlsx");
        ...
        'filename', currentFile, 'fs',fs, 'dwt_c', c, 'dwt_l', l, 'approx4', approx4, 'detail1', detail1, 'detail2', detail2, 'detail3', detail3);  % Almacenar el nombre del archivo
